...
namespace IcsManagerLibrary
{
	...
	{
		[Parameter(HelpMessage = "Connection to share (name or GUID)", Mandatory = true, Position = 0)]
		public string Shared_connection;

		[Parameter(HelpMessage = "Home connection (name or GUID)", Mandatory = true, Position = 1)]
		public string Home_connection;

		[Parameter(HelpMessage = "Force disabling ICS if already enabled", Mandatory = false)]
